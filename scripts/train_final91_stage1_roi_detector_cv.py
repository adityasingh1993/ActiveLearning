...
def run(args):
    ...
    def component_crop(probability):
        raw = probability >= args.detector_threshold
        labeled, count = ndimage.label(raw)
        if count == 0:
            shape = np.asarray(raw.shape, dtype=int)
            return np.zeros(3, dtype=int), shape - 1, 0, 1
        sizes = np.bincount(labeled.ravel())[1:]
        minimum = max(args.min_component_voxels, int(math.ceil(float(sizes.max()) * args.min_component_relative_size)))
        ...
        lo, hi = bounds
        lo, hi = expand_bounds(lo, hi, raw.shape, args.inference_margin)
    ...
    def train_fold(config_template, by_id, spec, output_dir, device):
        ...
        train_loader = make_loader([by_id[x] for x in spec["train_ids"]], train_t, True, config_template)
    ...
    for fold in selected_folds:
        fresh_rows.extend(train_fold(config, by_id, fold_map[fold], output_dir, device))

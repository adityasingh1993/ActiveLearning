...
def subset_summary(indices, actual_dice, actual_failure, actual_high_quality):
    ...
    if n == 0:
        return {
            "n": 0, "mean_actual_dice": float("nan"), "median_actual_dice": float("nan"),
...
def threshold_values(values):
    return [0.0] + sorted(set(float(x) for x in values)) + [1.0]


def search_acceptance_candidates(actual_dice, actual_failure, actual_high_quality,
                                 predicted_dice, failure_prob, folds, max_failure_rate,
                                 min_mean_dice, min_high_quality_rate, min_accepted):
    n_total = len(actual_dice)
    candidates = []
    for p_thr in threshold_values(failure_prob):
        for d_thr in threshold_values(predicted_dice):
            ...
            for fold in sorted(set(int(x) for x in folds)):
                ...
                stats = subset_summary(fold_idx, actual_dice, actual_failure, actual_high_quality)
                fold_counts[str(fold)] = stats["n"]
                fold_failures[str(fold)] = stats["observed_failures"]
            candidates.append({
    ...
    candidates.sort(key=lambda r: (-r["coverage"], r["accept_failure_probability_max"],
                                   -r["accept_predicted_dice_min"]))
    return candidates


def assign_three_way_policy(predicted_dice, failure_prob, accept_p, accept_dice,
                            active_p, active_dice):
    decisions = []
...
def main():
    parser = argparse.ArgumentParser(description="Development risk/coverage analysis for QC-gated auto-labeling")
    parser.add_argument("--predictions", default=str(DEFAULT_DIR / "qc_oof_predictions.csv"))
    ...
    parser.add_argument("--active-failure-prob", type=float, default=0.50)
    parser.add_argument("--active-predicted-dice", type=float, default=0.70)
    ...
    required = {"fold", "case_id", "actual_dice", "predicted_dice_oof", "actual_failure",
                "predicted_failure_probability_oof", "actual_high_quality"}
    ...
    actual_high_quality = np.asarray([int(r["actual_high_quality"]) for r in rows])

    if not (np.isfinite(actual_dice).all() and np.isfinite(predicted_dice).all()
            and np.isfinite(failure_prob).all()):
        raise RuntimeError("Predictions contain non-finite values")

    risk_rows = risk_coverage_rows(actual_dice, actual_failure, actual_high_quality, failure_prob)
    candidates = search_acceptance_candidates(
        actual_dice, actual_failure, actual_high_quality, predicted_dice, failure_prob, folds,
        args.max_accepted_failure_rate, args.min_accepted_mean_dice,
        args.min_accepted_high_quality_rate, args.min_accepted)
    ...
    if candidates:
        accept_p = float(candidates[0]["accept_failure_probability_max"])
        accept_dice = float(candidates[0]["accept_predicted_dice_min"])
        policy_source = "max-coverage candidate satisfying development constraints"
    else:
        accept_p, accept_dice = -1.0, 1.1
        policy_source = "no candidate satisfied constraints; auto-accept disabled"

    decisions = assign_three_way_policy(predicted_dice, failure_prob, accept_p, accept_dice,
                                        args.active_failure_prob, args.active_predicted_dice)

    decision_rows = []
    for i in range(len(rows)):
        decision_rows.append({
            "fold": int(folds[i]), "case_id": str(case_ids[i]),
    ...
    for bucket in ["AUTO_ACCEPT", "REVIEW", "ACTIVE_LEARN_PRIORITY"]:
        bucket_metrics[bucket] = subset_summary(np.where(decisions == bucket)[0], actual_dice,
                                                actual_failure, actual_high_quality)

    failure_indices = np.where(actual_failure == 1)[0]
    missed_failure_ranking = [{
        "case_id": str(case_ids[i]), "fold": int(folds[i]), "actual_dice": float(actual_dice[i]),
        "predicted_dice_oof": float(predicted_dice[i]),
        "predicted_failure_probability_oof": float(failure_prob[i]),
        "development_policy_decision": str(decisions[i]),
    ...
    metadata = {
        ...
        "warning": "Policy thresholds and metrics are selected/evaluated on the same 47-case development OOF QC predictions. Validate on a future locked set before production auto-accept.",
        ...
        "selected_development_policy": {
            ...
            "auto_accept_if": {"predicted_failure_probability_lte": accept_p,
    ...
    print(f"Acceptance search constraints: failure_rate<={args.max_accepted_failure_rate:.3f}, mean Dice>={args.min_accepted_mean_dice:.3f}, high-quality rate>={args.min_accepted_high_quality_rate:.3f}")
    print("=" * 100)
    if candidates:
        top = candidates[0]
        print("\nSELECTED DEVELOPMENT AUTO-ACCEPT GATE")
        print(f"P(failure) <= {float(top['accept_failure_probability_max']):.4f} AND predicted Dice >= {float(top['accept_predicted_dice_min']):.4f}")
        print(f"Coverage={float(top['coverage']):.3f} ({int(top['accepted_n'])}/{len(rows)}) | observed failures={int(top['observed_failures'])} | mean actual Dice={float(top['mean_actual_dice']):.4f} | actual high-quality rate={float(top['high_quality_rate']):.3f}")
        print(f"95% Wilson upper bound on accepted failure rate: {float(top['failure_rate_wilson95_upper']):.3f}")
    else:
    ...
    for bucket in ["AUTO_ACCEPT", "REVIEW", "ACTIVE_LEARN_PRIORITY"]:
        ...
        if stats["n"] == 0:
            ...
            print(f"{bucket:<22} n={stats['n']:2d} | mean Dice={stats['mean_actual_dice']:.4f} | failures={stats['observed_failures']} | HQ rate={stats['high_quality_rate']:.3f}")

    print("\nLowest QC-risk ACTUAL failures (the cases most dangerous to auto-accept):")
    for row in missed_failure_ranking[:5]:
        print(f"  fold {row['fold']} {row['case_id']} | actual Dice={row['actual_dice']:.4f} | pred Dice={row['predicted_dice_oof']:.4f} | P(fail)={row['predicted_failure_probability_oof']:.4f} | {row['development_policy_decision']}")

    print("\nOutputs:")
    for path in [risk_path, candidate_path, decision_path, metadata_path]:
        print(f"  {path}")

#!/usr/bin/env python3
"""Run the frozen 40-label supervised CV experiment with translation-only augmentation.

Preparation prerequisite:
  python scripts/prepare_40_labeled_cv.py --config <config>

That command must create and freeze:
  experiments/cv5_supervised_40_translation12/cv_splits.json

Training profile:
- all labeled cases visible to the config (manifest currently expected to contain 40)
...
OUTPUT_DIR = Path("experiments/cv5_supervised_40_translation12")
...
def main():
    if not SPLIT_MANIFEST.exists():
        raise FileNotFoundError(
            f"Missing frozen 40-label split manifest: {SPLIT_MANIFEST}\n"
            "Run first: python scripts/prepare_40_labeled_cv.py --config <config>"
        )
    ...
    def run_fold_with_experiment_label(args, fold_spec, output_dir):
        ...
        for row in rows:
            row["experiment"] = "40labels_translation12"
        return rows
    ...
    print(
        "40-label profile: frozen 5-fold manifest | translation +/-12 vox (p=0.8) | "

...
Run this before the 40-label supervised experiment. It intentionally does not use
...
- verifies the expected labeled-case count (40 by default)
...
Outputs default to:
  experiments/cv5_supervised_40_translation12/dataset_audit.csv
  experiments/cv5_supervised_40_translation12/audit_summary.json
  experiments/cv5_supervised_40_translation12/fold_summary.csv
  experiments/cv5_supervised_40_translation12/cv_splits.json
...
DEFAULT_OUTPUT_DIR = Path("experiments/cv5_supervised_40_translation12")
...
def main():
    parser = argparse.ArgumentParser(
        description="Audit 40 labeled cases and freeze the 5-fold manifest for translation-only CV"
    )
    ...
    if len(cases) != args.expected_cases:
        raise RuntimeError(
            ...
            "Resolve missing/extra image-label pairs before freezing the 40-case CV split."
        )

    print("=" * 100)
    print("40-LABEL DATASET PREPARATION")
    ...
    if focus_summary is not None:
        ...
        print(f"\nFocus case {args.focus_case} is not present in the current 40 labeled cases.")

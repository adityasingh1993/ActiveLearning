#!/usr/bin/env python3
"""Train the final all-62 residual guided ROI refiner.

This is the fair 62-case counterpart of train_residual_guided_refiner_oof47_2ch.py.
It reuses the exact same residual architecture, recipe, fixed epoch rule, and predicted-ROI
construction, but trains on 62 leakage-safe OOF ROI examples instead of 47.

Inputs
------
channel 0: ultrasound predicted-ROI crop
channel 1: leakage-safe all-62 OOF EMA probability crop

Residual rule:
    corrected_logit = logit(coarse_probability) + delta_logit

Production remains one-stage; this model is offline-only.
"""
...
DEFAULT_DATASET_DIR = Path("experiments/guided_refiner_oof62_roi_dataset_v1")
DEFAULT_REFERENCE_CV_DIR = Path("experiments/round2_cv_62_translation12")
DEFAULT_OUTPUT_DIR = Path("experiments/residual_guided_refiner_oof62_2ch_v1")
EXPECTED_CASES = 62


def main():
    p = argparse.ArgumentParser(description="Train residual 2-channel guided OOF62 ROI refiner")
    p.add_argument("--config", required=True)
    p.add_argument("--dataset-dir", default=str(DEFAULT_DATASET_DIR))
    p.add_argument("--reference-cv-dir", default=str(DEFAULT_REFERENCE_CV_DIR))
    ...
    manifest_path = dataset_dir / "guided_refiner_oof62_roi_manifest.csv"
    metadata_path = dataset_dir / "guided_refiner_oof62_roi_metadata.json"
    rows = read_csv(manifest_path)
    if len(rows) != EXPECTED_CASES or len({str(r["case_id"]) for r in rows}) != EXPECTED_CASES:
        raise RuntimeError("Residual guided refiner dataset must contain exactly 62 unique OOF cases")
    ...
    # Keep the same fixed epoch rule used for the OOF47 residual experiment, so the only intended
    # change is 47 -> 62 leakage-safe training examples.
    epoch_rows = read_cv_best_epochs(Path(args.reference_cv_dir))
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        if args.overwrite:
            ...
            print(f"Residual OOF62 checkpoint already exists: {final_checkpoint}")
    ...
    config.experiment_name = f"{config.experiment_name}-residual-guided-oof62-2ch"
    ...
    print("=" * 120)
    print("RESIDUAL 2-CHANNEL GUIDED ROI REFINER — OOF62 FINAL FAIR ROI EXPERIMENT")
    print(f"Cases:               {len(cases)} true-OOF predicted ROIs")
    print("Channels:            ultrasound + all-62 OOF EMA probability")
    ...
    print(f"Reference best epochs: {best_epochs.tolist()}")
    print(f"Fixed final epochs:  {final_epochs}")
    print(f"Physical GPU:        {SELECTED_GPU if SELECTED_GPU is not None else '<environment/config>'}")
    print("Recipe:              SAME as OOF47 residual | 2ch DynUNet | resize128 | DiceCE | AdamW 1e-4 | EMA")
    print("Augmentation:        paired image/coarse/label translation +/-12 vox, p=0.8")
    print("Controlled change:   residual training examples 47 -> 62 only")
    ...
    training_meta = {
        "version": "residual_guided_refiner_oof62_2ch_v1",
        "purpose": "final fair all-62 offline ROI residual-correction experiment",
        "n_training_cases": len(cases),
        "source_dataset": str(dataset_dir),
        "input_channels": ["ultrasound", "leakage_safe_all62_oof_ema_probability"],
        "in_channels": 2,
        ...
        "seed": int(args.seed),
        "reference_best_epochs": [int(x) for x in best_epochs.tolist()],
        ...
        "controlled_change_from_oof47_residual": "47 -> 62 leakage-safe OOF training examples",
        ...
        "warning": (
            "This residual 2-channel model is an OFFLINE refiner experiment. Production remains a "
            "single-stage segmentation model."
        ),
    }
    ...
    print("\nRESIDUAL OOF62 GUIDED REFINER TRAINING COMPLETE")

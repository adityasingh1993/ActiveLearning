#!/usr/bin/env python3
"""Weighted controlled 128^3 SSL pretraining for the Final62 comparison.

This is the v2 controlled SSL run after observing raw equal-weight loss imbalance.
Everything from the leakage-safe Final62 SSL experiment is preserved except task weighting:

    inpainting   1.0
    rotation     0.25
    contrastive  5.0

The stronger 4x4x4 spatial-token contrastive views remain unchanged.
"""
...
from hassl.ssl.weighted_pretrainer import WeightedSSLPretrainer
...
# Keep the same output namespace so the existing Final62 SSL fine-tuner can consume the result.
...
def main():
    p = argparse.ArgumentParser(
        description="Leakage-safe weighted SSL pretraining for Final62"
    )
    ...
    p.add_argument("--lambda-rotation", type=float, default=0.25)
    p.add_argument("--lambda-contrastive", type=float, default=5.0)
    args = p.parse_args()
    ...
    # v2 weighted SSL settings. These are runtime attributes intentionally isolated to this
    # experiment branch; the shared HASSLConfig schema remains unchanged.
    ...
    config.experiment_name = f"{config.experiment_name}-controlled-ssl-final62-128-weighted"
    ...
    audit_cache = (
        Path(tempfile.gettempdir()) / "hassl_controlled_ssl_final62_128_weighted_audit"
        if args.audit_only
    ...
    print("=" * 120)
    print("CONTROLLED FINAL62 SSL PRETRAINING — 128^3 — WEIGHTED V2")
    ...
    print("Contrastive:              4x4x4 tokens + aligned gamma/scale/shift/noise views")
    print(
        "SSL task weights:          "
    ...
    if args.audit_only:
        audit_output = output_dir.parent / f"{output_dir.name}_weighted_pool_audit_preview.json"
    ...
    # Runtime-only experimental attributes are not part of HASSLConfig.to_dict().
    tracker.log_metrics(
        {
            ...
            "ssl_contrastive_grid_size": float(config.ssl_contrastive_grid_size),
        },
        step=0,
    )

    pretrainer = WeightedSSLPretrainer(
        config=config, dataloader=loader, tracker=tracker
    )
    pretrainer.train(num_epochs=config.ssl_epochs)
    tracker.finish()

    if not legacy_checkpoint.exists():
        raise FileNotFoundError(f"WeightedSSLPretrainer did not create {legacy_checkpoint}")

    state = torch.load(legacy_checkpoint, map_location="cpu", weights_only=False)
    metadata = {
        "version": "controlled_final62_ssl128_weighted_v2",
        ...
        "ssl_contrastive_grid_size": int(config.ssl_contrastive_grid_size),
        "ssl_contrastive_tokens_per_volume": int(config.ssl_contrastive_grid_size ** 3),
        "contrastive_view": {
            "gamma": [0.8, 1.2],
            "intensity_scale": [0.9, 1.1],
            "intensity_shift_abs": 0.05,
            "gaussian_noise_std": [0.0, 0.05],
            "spatial_transform": "none; token correspondence preserved",
        },
        ...
        "transfer_rule": (
            ...
            "output_block/deep_supervision_heads, then synchronizes EMA teacher to the "
    ...
    print("\nCONTROLLED WEIGHTED SSL PRETRAINING COMPLETE")

#!/usr/bin/env python3
"""Build the first 2-channel guided-refiner training dataset from existing Round2 OOF47 maps.

For each original-47 case:
- channel 1 source: native ultrasound image,
- channel 2 source: leakage-safe OOF Round2 EMA probability map,
- ROI: proposed from that OOF probability map using the frozen training-only localization prior,
- target: human GT cropped to exactly the same ROI.

The ROI is NOT corrected with GT. GT coverage is measured only as a diagnostic so the training
input distribution matches the predicted-ROI inference setting as closely as possible.
"""
...
try:
    ...
    raise ImportError("build_guided_refiner_oof47_roi_dataset.py requires SimpleITK") from exc
...
DEFAULT_OOF_MANIFEST = Path(
    "experiments/guided_refiner_existing_round2_oof47_probs_v1/"
    "existing_round2_oof47_ema_probability_manifest.csv"
)
DEFAULT_AUDIT_CSV = Path(
    "experiments/round2_supervised_62_translation12/round2_label_audit.csv"
)
DEFAULT_OUTPUT_DIR = Path("experiments/guided_refiner_oof47_roi_dataset_v1")
EXPECTED_CASES = 47
...
def main():
    p = argparse.ArgumentParser(description="Build guided-refiner OOF47 predicted-ROI dataset")
    ...
    if len({str(r["case_id"]) for r in source_rows}) != EXPECTED_CASES:
        raise RuntimeError("OOF47 manifest contains duplicate case IDs")

    # Deployment has all 62 audited HUMAN_GOLD labels available to estimate this global prior.
    # The prior never sees external GT and does not alter the OOF coarse probability map.
    ...
    manifest_path = output_dir / "guided_refiner_oof47_roi_manifest.csv"
    ...
    metadata = {
        "version": "guided_refiner_oof47_predicted_roi_dataset_v1",
        "n_cases": len(rows),
    ...
    metadata_path = output_dir / "guided_refiner_oof47_roi_metadata.json"
    metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")

    print("\n" + "=" * 112)
    print("GUIDED REFINER OOF47 PREDICTED-ROI DATASET COMPLETE")

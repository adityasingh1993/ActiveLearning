#!/usr/bin/env python3
"""Evaluate the single final all90 CenterNet -> Stage-2 pipeline on External31.

This is the deployment-aligned follow-up to the five-crop committee diagnostic. One CenterNet
produces one crop; one Student/EMA Stage-2 model predicts it; the probability is pasted into the
full 128^3 grid and exactly inverted to native geometry. RAW @ 0.50 is primary and fixed
26-connected LCC is diagnostic. No external-data tuning is performed.
"""
...
STAGE2_FINAL = Path("experiments/final91_qc_two_stage_all90/stage2")
STAGE2_CV = Path("experiments/final91_qc_stage2_centernet_dynunet_cv")
OUTPUT = Path("experiments/external31_final91_qc_two_stage_all90_locked")
FULL_VOLUME_FINAL91 = Path("experiments/external31_final91_a3_locked/external31_case_metrics.csv")
COMMITTEE_METRICS = Path(
    "experiments/external31_final91_qc_two_stage_cv_committee_locked/"
    "external31_two_stage_raw_case_metrics.csv"
)
...
def compare(current, baseline_path, baseline_label, output_dir, filename):
    ...
    if set(before) != set(after) or len(after) != EXPECTED_EXTERNAL:
        raise RuntimeError(f"{baseline_label} and all90 two-stage metrics do not contain identical IDs")
    rows = []
    for case_id in sorted(after):
        ...
        rows.append({
            ...
            "all90_two_stage_dice": float(after[case_id]["dice"]),
            "delta_dice": delta,
            "baseline_precision": float(before[case_id]["precision"]),
            "all90_two_stage_precision": float(after[case_id]["precision"]),
            "baseline_recall": float(before[case_id]["recall"]),
            "all90_two_stage_recall": float(after[case_id]["recall"]),
        })
    ...
    summary = {
        ...
        "all90_two_stage_mean_dice": float(np.mean([row["all90_two_stage_dice"] for row in rows])),
...
def dry_run(args):
    print("=" * 116)
    print("FINAL91-QC FINAL ALL90 TWO-STAGE — EXTERNAL31 DRY RUN")
    print("Pipeline:             one final CenterNet -> one Stage-2 Student+EMA")
    ...
    print("Comparisons:          full-volume Final91 and five-crop committee")
...
def run(args):
    ...
    if not audit.get("all_visible_labels_passed_audit", False) or len(training_ids) != 91:
        raise RuntimeError("Final91 audit is not passing or does not contain exactly 91 IDs")
    if QUARANTINED_CASE_ID not in training_ids:
        raise RuntimeError("Expected quarantine is absent from the Final91 audit")
    ...
    expected_train_ids = sorted(set(training_ids) - {QUARANTINED_CASE_ID})
    if (
        ...
        raise RuntimeError("Final two-stage checkpoints are not trained on the exact same all90 set")
    if (
    ...
    primary_path = output_dir / "external31_all90_two_stage_raw_case_metrics.csv"
    ...
    print("=" * 116)
    print("FINAL91-QC FINAL ALL90 TWO-STAGE — LOCKED EXTERNAL31")
    ...
    write_csv(output_dir / "external31_all90_two_stage_lcc_case_metrics.csv", lcc_rows)
    write_csv(output_dir / "external31_all90_localizer_diagnostics.csv", localizer_rows)
    write_csv(output_dir / "external31_all90_two_stage_summary.csv", [
    ...
    full_comparison = compare(
        ...
        "all90_two_stage_vs_fullvolume_final91",
    )
    committee_comparison = compare(
        raw_rows, args.committee_metrics, "FIVE_CROP_CV_COMMITTEE", output_dir,
    ...
    metadata = {
        ...
        "five_crop_committee_comparison": committee_comparison,
        "stage1_checkpoint": str(stage1_checkpoint),
        "stage2_checkpoint": str(stage2_checkpoint),
        "interpretation_role": (
            "predeclared deployment-aligned follow-up testing the mechanistic hypothesis that "
            "five independently zero-padded crops caused committee under-segmentation"
        ),
    }
    (output_dir / "external31_all90_two_stage_metadata.json").write_text(
        json.dumps(metadata, indent=2), encoding="utf-8"
    )

    print("\n" + "=" * 116)
    print("FINAL91-QC FINAL ALL90 TWO-STAGE — EXTERNAL31 RAW vs LCC")
    ...
    if full_comparison is not None:
        print(f"Vs full Final91:    {full_comparison['baseline_mean_dice']:.4f} -> "
              f"{full_comparison['all90_two_stage_mean_dice']:.4f} "
              f"({full_comparison['delta_mean_dice']:+.4f})")
    if committee_comparison is not None:
        print(f"Vs CV committee:    {committee_comparison['baseline_mean_dice']:.4f} -> "
              f"{committee_comparison['all90_two_stage_mean_dice']:.4f} "
              f"({committee_comparison['delta_mean_dice']:+.4f})")
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Evaluate final all90 Final91-QC two-stage pipeline on External31"
    )
    ...
    parser.add_argument("--committee-metrics", default=str(COMMITTEE_METRICS))

#!/usr/bin/env python3
"""Run the frozen 47-label supervised CV experiment with translation-only augmentation.

Preparation prerequisite:
  python scripts/prepare_47_labeled_cv.py --config <config>

That command must create and freeze:
  experiments/cv5_supervised_47_translation12/cv_splits.json

Training profile:
- all labeled cases visible to the config (manifest expected to contain 47)
...
OUTPUT_DIR = Path("experiments/cv5_supervised_47_translation12")
...
def main():
    if not SPLIT_MANIFEST.exists():
        raise FileNotFoundError(
            f"Missing frozen 47-label split manifest: {SPLIT_MANIFEST}\n"
            "Run first: python scripts/prepare_47_labeled_cv.py --config <config>"
        )
    ...
    def run_fold_with_experiment_label(args, fold_spec, output_dir):
        ...
        for row in rows:
            row["experiment"] = "47labels_translation12"
        return rows
    ...
    print(
        "47-label profile: frozen 5-fold manifest | translation +/-12 vox (p=0.8) | "

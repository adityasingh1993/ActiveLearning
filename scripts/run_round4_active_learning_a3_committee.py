#!/usr/bin/env python3
"""Round-4 active-learning triage using the current Final72 A3 fold committee.

Purpose
-------
Resume HUMAN_GOLD acquisition after Final72 model-screening experiments. This script does NOT
train a segmentation model and does NOT use external31 GT for selection.

The current A3 model is represented by the five completed frozen-fold checkpoints from:
    experiments/final72_screen_a3_translation4_p05_lrflip_p05/checkpoints/fold_{0..4}

Every remaining unlabeled pool case is unseen by all five folds. For each fold we run the exact
raw Student+EMA 50/50 ensemble at threshold 0.50, extract deployment-only prediction features,
and aggregate those features across the five-model committee.

Three operational states are produced:

KEEP_CANDIDATE
    The case was already HIGH_CONFIDENCE_PSEUDO_LABEL under the previously locked/calibrated
    Final62 Scenario-F pool run. This script PRESERVES that old calibrated decision only; it does
    not recalibrate QC on A3 and it never promotes the pseudo-label into training automatically.

ANNOTATE
    A non-KEEP case selected for new HUMAN_GOLD annotation using A3 committee uncertainty,
    low-volume/localization/boundary-oversegmentation proxies, and robust feature diversity.

REVIEW
    Remaining non-KEEP cases. They stay in the unlabeled review pool.

Selection profiles (default batch size 10)
------------------------------------------
- 4 SMALL_UNCERTAIN: low predicted foreground burden + uncertainty/instability
- 3 LOCALIZATION_UNSTABLE: centroid/volume/component instability across folds
- 3 BOUNDARY_OVERSEG_PROXY: high foreground/boundary uncertainty/fragmentation
Any unfilled slots fall back to general committee risk + diversity.

Safeguards
----------
- Exact passing Final72 audit is required (72 HUMAN_GOLD IDs).
- All 72 current HUMAN_GOLD IDs are excluded.
- Frozen external31 IDs are excluded. Only their IDs are read; no external GT/metrics enter scores.
- Previously-selected-but-unresolved Round2 cases are excluded by default to avoid repeatedly
  spending annotation budget on cases intentionally skipped earlier. Use --include-previously-skipped
  only if you explicitly want them back in the candidate pool.
- The old Scenario-F HIGH_CONFIDENCE status is used only for KEEP_CANDIDATE. A3 features are NOT
  fed through the old QC regressors/classifiers because that calibration belongs to another
  prediction definition.
- No selected AI prediction is called HUMAN_GOLD.

Default output
--------------
experiments/round4_active_a3_committee_v1/
    committee_fold_features.csv
    round4_pool_triage.csv
    round4_annotation_batch.csv
    round4_keep_candidates.csv
    round4_review_queue.csv
    round4_metadata.json
    annotation_pack/               (only with --materialize)
        01_<case_id>/image/<case>.mha
        01_<case_id>/PROVENANCE.json

Example
-------
python scripts/run_round4_active_learning_a3_committee.py \
...
OUTPUT_DIR = Path("experiments/round4_active_a3_committee_v1")
...
def aggregate_case_features(case_id, fold_rows, image_path):
    if len(fold_rows) != 5:
        raise RuntimeError(f"{case_id}: expected five A3 fold feature rows, got {len(fold_rows)}")

    ignored = {"case_id", "fold", "image_path"}
    numeric_keys = sorted(set.intersection(*[set(r.keys()) for r in fold_rows]) - ignored)
    out = {"case_id": case_id, "image_path": image_path}
    for key in numeric_keys:
        vals = np.asarray([as_float(r.get(key)) for r in fold_rows], dtype=float)
        finite = vals[np.isfinite(vals)]
        out[f"mean_{key}"] = float(np.mean(finite)) if finite.size else float("nan")
        out[f"std_{key}"] = float(np.std(finite)) if finite.size else float("nan")

    centroid_cols = [
        np.asarray([as_float(r.get("centroid_d_norm")) for r in fold_rows], dtype=float),
        np.asarray([as_float(r.get("centroid_h_norm")) for r in fold_rows], dtype=float),
        np.asarray([as_float(r.get("centroid_w_norm")) for r in fold_rows], dtype=float),
    ]
    spreads = []
    valid_axes = 0
    for arr in centroid_cols:
        finite = arr[np.isfinite(arr)]
        if finite.size >= 2:
            spreads.append(float(np.std(finite)))
            valid_axes += 1
        elif finite.size == 1:
            spreads.append(0.0)
            valid_axes += 1
    ...
    if valid_axes == 0:
        # No fold predicted foreground: treat as maximal localization uncertainty later.
        out["centroid_spread"] = 1.0
    else:
        clean = [x for x in spreads if math.isfinite(x)]
        out["centroid_spread"] = float(np.sqrt(np.sum(np.square(clean)))) if clean else 1.0

    mean_fg = as_float(out.get("mean_pred_fg_fraction"))
    std_fg = as_float(out.get("std_pred_fg_fraction"))
    out["pred_fg_cv"] = (
        float(std_fg / max(mean_fg, 1e-6))
        if math.isfinite(mean_fg) and math.isfinite(std_fg)
        else float("nan")
    )
    return out


def choose_from_pool(eligible, selected, category_score, risk_score, distances, quota, risk_weight, case_ids):
    available = [i for i in eligible if i not in selected]
    ...
    local = np.asarray([category_score[i] for i in available], dtype=float)
    local_norm = minmax(local)
    norm_by_idx = {i: float(local_norm[pos]) for pos, i in enumerate(available)}

    while len(chosen) < quota:
        remaining = [i for i in available if i not in chosen]
        anchors = list(selected) + list(chosen)
        if anchors:
            raw_dist = np.asarray([float(np.min(distances[i, anchors])) for i in remaining], dtype=float)
            div_norm = minmax(raw_dist)
        else:
            raw_dist = np.full(len(remaining), np.nan, dtype=float)
            div_norm = np.ones(len(remaining), dtype=float)

        scored = []
        for pos, i in enumerate(remaining):
            combined = risk_weight * norm_by_idx[i] + (1.0 - risk_weight) * float(div_norm[pos])
            scored.append((i, combined, float(div_norm[pos]), float(raw_dist[pos])))
        i, combined, dnorm, draw = sorted(
            scored,
            key=lambda x: (-x[1], -category_score[x[0]], -risk_score[x[0]], str(case_ids[x[0]])),
        )[0]
        chosen.append(i)
        records.append({
            "index": int(i),
            "combined_selection_score": float(combined),
            "normalized_diversity_score": float(dnorm),
            "min_distance_to_previous": draw,
        })
    return chosen, records
...
def materialize_images(output_dir: Path, selected_rows):
    ...
    for row in selected_rows:
        ...
        provenance = {
            ...
            "selection_rank": rank,
            "round": 4,
            "selection_model": "Final72 A3 five-fold committee",
...
def main():
    p = argparse.ArgumentParser(description="Round4 active-learning selection with Final72 A3 committee")
    ...
    p.add_argument("--small-quota", type=int, default=4)
    p.add_argument("--localization-quota", type=int, default=3)
    ...
    p.add_argument("--materialize", action="store_true", help="Copy selected source images into an annotation pack")
    ...
    if abs(float(args.threshold) - 0.50) > 1e-8:
        p.error("Round4 A3 committee screening is locked to threshold 0.50")
    ...
    if config.compute_mode != "prototype" or config.unet_backbone != "dynunet":
        raise RuntimeError("Round4 A3 committee requires prototype DynUNet Student+EMA mode")
    cv.apply_baseline(config, args.resize_size, epochs=1)

    final72_audit = read_json(Path(args.final72_audit))
    if not final72_audit.get("all_visible_labels_passed_audit", False):
        raise RuntimeError("Final72 audit is not passing")
    human_ids = set(str(x) for x in final72_audit.get("all_current_human_label_ids", []))
    if len(human_ids) != 72:
        raise RuntimeError(f"Expected exactly 72 audited HUMAN_GOLD IDs, found {len(human_ids)}")
    ...
    if human_ids & external_ids:
        raise RuntimeError("Final72 HUMAN_GOLD overlaps frozen external31")

    skipped_ids = set()
    round2_audit_path = Path(args.round2_audit)
    if round2_audit_path.exists():
        r2 = read_json(round2_audit_path)
        skipped_ids = set(str(x) for x in r2.get("selected_unlabeled_ids", []))
    ...
    print("=" * 124)
    print("ROUND 4 ACTIVE LEARNING — FINAL72 A3 FIVE-FOLD COMMITTEE")
    ...
    print("KEEP status comes ONLY from the old locked Scenario-F manifest; A3 is acquisition-only.")
    print("=" * 124)
    ...
    for case_id in sorted(per_case):
        aggregated.append(aggregate_case_features(case_id, per_case[case_id], case_path[case_id]))

    # Preserve calibrated Final62 KEEP status, but never infer new KEEP status from A3 features.
    ...
    boundary_entropy = np.asarray([as_float(r.get("mean_boundary_entropy_mean")) for r in acquisition], dtype=float)
    ...
    loc_eligible = [
        ...
        if centroid_spread[i] >= centroid_cut
        or component_std[i] > comp_std_cut
        or lcf[i] < 0.90
    ]
    boundary_cut = float(np.quantile(boundary_entropy, 0.60))
    boundary_eligible = [
        i for i in range(len(acquisition))
        if pred_fg[i] >= high_cut
        or boundary_entropy[i] >= boundary_cut
        or component_mean[i] >= 2.0
    ]
    ...
    # Attach scores to acquisition rows.
    ...
    # KEEP rows were excluded from A3 selection but retain aggregated A3 diagnostics for review.
    keep_rows = [r for r in aggregated if int(r["locked_keep_candidate"]) == 1]
    for row in keep_rows:
        row["round4_state"] = "KEEP_CANDIDATE"
        row["selection_rank"] = ""
    ...
    annotation_rows = sorted(
        [r for r in acquisition if r["round4_state"] == "ANNOTATE"],
        key=lambda r: int(r["selection_rank"]),
    )
    review_rows = sorted(
        [r for r in acquisition if r["round4_state"] == "REVIEW"],
    ...
    write_csv(output_dir / "round4_annotation_batch.csv", annotation_rows)
    write_csv(output_dir / "round4_review_queue.csv", review_rows)
    write_csv(output_dir / "round4_keep_candidates.csv", keep_rows)
    write_csv(output_dir / "round4_pool_triage.csv", triage_rows)

    pack = materialize_images(output_dir, annotation_rows) if args.materialize else None

    profile_counts = {}
    for row in annotation_rows:
        pkey = str(row["selection_profile"])
        profile_counts[pkey] = profile_counts.get(pkey, 0) + 1

    metadata = {
        "version": "round4_a3_committee_acquisition_v1",
        "selection_is_training": False,
        "current_human_gold_count": len(human_ids),
        "current_human_gold_source": str(args.final72_audit),
        "a3_committee_dir": str(a3_dir),
        "a3_fold_checkpoints": [str(x) for x in checkpoints],
        ...
        "risk_formula": (
            "rank blend of committee entropy, student-EMA disagreement, fold-to-fold foreground instability, "
            "centroid spread, uncertain fraction, and fragmentation"
        ),
    ...
    (output_dir / "round4_metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")

    print("\n" + "=" * 124)
    print("ROUND 4 TRIAGE RESULT")
    print("=" * 124)
    ...
    print(f"\nTriage CSV:     {output_dir / 'round4_pool_triage.csv'}")
    print(f"Annotation CSV: {output_dir / 'round4_annotation_batch.csv'}")
    print(f"KEEP CSV:       {output_dir / 'round4_keep_candidates.csv'}")
    print(f"Review CSV:     {output_dir / 'round4_review_queue.csv'}")
    if pack:
        print(f"Annotation pack:{pack}")
    print("\nKEEP is still candidate-only. Do not add KEEP predictions to HUMAN_GOLD training automatically.")
    print("=" * 124)

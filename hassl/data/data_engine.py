...
def get_or_create_frozen_splits(
    ...
    n_val_p = min(val_split, max(0, remaining_after_test - 1))
    if n_total_p < 5:
...
def get_base_transforms(config, keys=["image", "label"], is_training: bool = False, apply_strong_aug: bool = True):
    ...
    if preprocessing_mode == "patch":
        ...
        # Preserve post-Spacingd scale for validation. Padding guarantees the inference
        # window fits but does not alter voxel size or globally resample the anatomy.
